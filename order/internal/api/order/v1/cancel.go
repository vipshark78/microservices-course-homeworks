...
func (a *api) OrderCancel(ctx context.Context, params order_v1.OrderCancelParams) (order_v1.OrderCancelRes, error) {
	...
	if err != nil {
		if errors.Is(err, model.ErrOrderCancelled) {
			return &order_v1.ConflictError{Code: http.StatusConflict, Message: "Order has already been cancelled"}, nil
		}
		...
		if errors.Is(err, model.ErrOrderPaid) {
			return &order_v1.ConflictError{Code: http.StatusConflict, Message: "Order has already paid and cannot be cancelled"}, nil
		}
